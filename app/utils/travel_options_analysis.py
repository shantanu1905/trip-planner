...
def analyze_trip_options(legs_data: dict) -> dict:
    """
    Analyze all legs (Train, Bus, Flight, etc.) and attach respective analysis inside each leg.
    Output format matches user's requested structure.
    """
    option_name = legs_data.get("option_name", "Unknown Route")
    legs = legs_data.get("legs", [])
    ...
    for leg in legs:
        ...
        # Copy the original leg data for enrichment
        analyzed_leg = leg.copy()
        ...
        if mode == "bus":
            try:
                ...
                # Format date properly for external APIs
                try:
                    formatted_date = datetime.strptime(journey_date, "%Y-%m-%d").strftime("%d-%m-%Y")
                except:
                    formatted_date = journey_date
        ...
        # --- ✈️ FLIGHT MODE ---
            ...
            try:
                print(f"✈️ Flight analysis placeholder: {from_city} → {to_city} on {journey_date}")

                analyzed_leg["flight_data_analysis"] = {
                    "status": False,
                    "message": "Flight analysis feature coming soon.",
                    "info": {
                        "from": from_city,
                        "to": to_city,
                ...
                analyzed_leg["flight_data_analysis"] = {
                    "status": False,
                    "error": str(e),
                    "message": "Flight analysis section failed (feature not active yet)."
                }
